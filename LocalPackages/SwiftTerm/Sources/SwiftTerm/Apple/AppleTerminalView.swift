...
extension TerminalView {
    ...
    func drawTerminalContents (dirtyRect: TTRect, context: CGContext, bufferOffset: Int)
    {
        ...
        // draw lines
        #if os(iOS) || os(visionOS)
        // On iOS the UIScrollView coordinate system means dirtyRect.minY can be 0
        // even when contentOffset.y > 0 (UIKit coalesces dirty rects and can deliver
        // a viewport-relative rect instead of a content-space rect). Using
        // contentOffset.y directly is always correct: it equals yDisp*cellHeight and
        // is never stale, unlike the dirty rect that may have been accumulated before
        // the last scroll position update.
        let cellHeight = cellDimension.height
